...
- `integration/`: application/process-boundary smoke checks; live PostgreSQL/MinIO coverage is
  ...
  `HIVEBLOT_RUN_LIVE_REVIEW_QUEUE=1`, or `HIVEBLOT_RUN_LIVE_WORKBENCH=1`. The queue suite seeds and
  removes 10,000 cases; the workbench suite verifies a direct signed MinIO image download.
