...
- `integration/`: application/process-boundary smoke checks; live PostgreSQL/MinIO coverage is
  ...
  `HIVEBLOT_RUN_LIVE_STRUCTURED_EDITOR=1`. The queue suite seeds and removes 10,000 cases; the
  workbench suite verifies a direct signed MinIO image download; the structured-editor suite checks
  strict JSONB round-trips, optimistic conflicts, and append-only undo.
